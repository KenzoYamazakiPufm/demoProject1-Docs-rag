...
SYSTEM_PROMPT = (
    ...
    "1. 只能依据下面提供的简历内容与任职记录作答；"
    "禁止使用它们之外的知识，禁止推测、禁止编造。\n"
    "2. 先尽力从这些内容里找线索：写了的内容要如实、完整地回答，不要无谓地拒答。\n"
    "3. 每一条结论后面都要标注来源编号，如 [1][2]。\n"
    "4. 遇到涉及年份或时间段的问题：下面给出的任职记录是本地数据库按时间区间"
    "精确计算出来的结果，必须直接采用，不要自己再去推算；"
    "引用它时把来源写成「他的任职记录」。\n"
    "5. 直接回答对方的问题，不要复述材料的标题或名称。"
    "需要说明依据时，用「他的简历」「他的任职记录」这类自然说法。\n"
    "6. 只有在简历里确实找不到与该问题相关的任何信息时，才回复：简历里没有写到这部分。"
)
...
NO_ANSWER_MARKERS = (
    "简历里没有写到这部分",      # 本次新增：模型被要求使用的标准话术
    "参考片段中没有相关信息",    # 保留兜底：模型未必严格照做，旧标记仍需有效
    "未找到相关内容", "没有相关信息",
